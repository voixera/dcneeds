...
async function __is_existing_mock_api(host: string, port: number): Promise<boolean> {
  try {
    ...
    return payload.ok === true && payload.service === "mock-bypass-api"
  } catch {
    return false
  }
}
...
export async function start_mock_bypass_api(): Promise<Server | null> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "EADDRINUSE") {
      if (await __is_existing_mock_api(host, port)) {
        console.info(`[ - MOCK BYPASS API - ] Using existing instance at http://${host}:${port}`)
        return null
      }
  ...
  console.info(`[ - MOCK BYPASS API - ] Running at http://${host}:${port}`)
  return __server
}

if (require.main === module) {
  start_mock_bypass_api().catch((error) => {
    console.error("[ - MOCK BYPASS API - ] Startup failed:", error)
    process.exit(1)
  })
}
